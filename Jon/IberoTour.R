...
sacarTimeDist(Ciudades,distancias,tiempo);
...
#Comienzo Zona Combinacion-----------------------------------------------------------------------------

trapMF=function(valor,conjunto){
  if(valor <= conjunto[1]){return(0)}
  if(valor > conjunto[1] && valor <= conjunto[2]){return((valor-conjunto[1])/(conjunto[2]-conjunto[1]))}
  if(valor > conjunto[2] && valor <= conjunto[3]){return(1)}
  if(valor > conjunto[3] && valor <= conjunto[4]){return((conjunto[4]-valor)/(conjunto[4]-conjunto[3]))}
  if(valor > conjunto[4]){return(0)}
}

km=c(0, 0, 200, 600)

cerquita = apply(data.frame(distancias[,1]), 1, FUN=function(x) trapMF(x,km))
for (ciudad in 2:ContCiudades) {
  cerquita = rbind(cerquita, apply(data.frame(distancias[,ciudad]), 1, FUN=function(x) trapMF(x,km)))
}

#Fin Zona Combinacion----------------------------------------------------------------------------------
